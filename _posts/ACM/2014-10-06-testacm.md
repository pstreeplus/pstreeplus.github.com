...
最简单的方法就是用栈模拟，时间复杂度最坏情况下是**O(len(s)*m)**，无疑会超时。用线段树，维护一个区间内有多少左括号和右括号，同时记录该区间已经匹配的长度。**向上更新的时候，左右括号直接加即可，对于更新已经匹配的长度：为孩子区间的相加，同时由于左区间的右括号和右区间的左括号会组成合法的匹配，也需要加上，取二者的最小值即可**。
