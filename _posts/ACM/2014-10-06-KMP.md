...
设fail[i]表示P[0...i]的lenMax，那么：如果P[i] = P[fail[i-1]]，则fail[i] = fail[i-1] + 1，如果P[i] != P[fail[i-1]],就另j = i-1,j = fail[j]，直到j = 0，这样求出的lenMax一定是最大的（这有点类似于动态规划的思想）。说起来那么麻烦，其实代码很短：
