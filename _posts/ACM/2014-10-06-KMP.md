...
设当前指针位置在T第i个字符上，匹配部分的字符串在T中为S1，在P中为S2，长度为len，显然S1 = S2。现在要做的就是移动S2，使S2的起始位置到T某个位置j (j <= i)，使得 S2[0...i-j] = S1[len-i+j-1...len-1]，由于S1=S2，所以S2[0...i-j] = S2[len-i+j-1...len-1]。即在S2的前缀与后缀相等子串中，找到子串的最大长度。例如abab的前缀有a,ab,aba,后缀有b,ab,bab。相等的只有ab，因此最大长度是2。
...
发现在最后一个字符处 d 与 a 不匹配，那么下一步该跳到哪呢？注意匹配的部分是**ccab**