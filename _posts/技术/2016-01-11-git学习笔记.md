...
####删除
> 一种是直接使用`rm file_name`直接在工作区删除，这样相当于是修改了工作区，此时可以使用`git checkout -- file_name`，撤销删除。如果确认删除的话就`add`然后再`commit`、`push`。

>另一种是通过`git rm file_name`，这样其实是相当于先`rm`再`add`，此时可以使用`git reset HEAD file_name`，撤销删除到工作区，再`git checkout -- file_name`恢复到工作区。或者确认删除就`commit`、`push`。
