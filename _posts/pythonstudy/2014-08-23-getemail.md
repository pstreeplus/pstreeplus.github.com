...
收取邮件最常用的协议是<let>POP</let>协议，目前版本号是<let>3</let>，俗称<let>POP3</let>。<let>Python</let>内置一个<let>poplib</let>模块可以实现邮件的收取。但是<let>POP3</let>协议收取的不是一个可读的邮件，而是邮件的原始文本

要把<Let>POP3</Let>收取的文本变成可以阅读的邮件，还需要用<let>email</let>模块提供的各种类来解析原始文本，变成可阅读的邮件对象。
...
实现的代码如下，其中有些内置函数，不了解用法可以去<let>Python</let>的官网查相关文档。
