...
5入栈后继续搜索,搜到3 (3,5为后向边),发现3被搜索过且在栈中, 于是low[5] = dfn[3] = 3 (dfn[5] != low[5])

<center><img src=/source/Graph/tarjan/l8.png></center>

返回4, 所以 low[4] = low[5] = 3 (dfn[4] != low[4]),继续返回至3, low[3] = 3, 发现 dfn[3] = low[3], 于是找到一个SCC, 3,4,5为一个SCC, 进行出栈操作:
